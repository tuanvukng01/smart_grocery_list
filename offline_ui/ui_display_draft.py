...
def capture_image_from_webcam():
    ...
    # Convert the captured frame to RGB and save it temporarily for PIL processing
    frame_rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
    image = Image.fromarray(frame_rgb)  # Convert the frame to PIL Image for processing
...
# Simulate detection and remove corresponding grocery item
def detect_and_remove_item(interpreter, class_labels, root):
    image = capture_image_from_webcam()  # Capture image from webcam
    if image is None:
        print("Image capture failed. No action taken.")
        return
    ...
    if predicted_class_label != "Unknown":
        ...
        # Get the current list of grocery items
        grocery_items = grocery_list.get_list()
        ...
        for item, quantity in grocery_items:
            if item == predicted_class_label:
                if quantity > 1:
                    ...
                    grocery_list.add_item(predicted_class_label, -1)  # Decrease by 1
        ...
        # Recreate UI after modification
        recreate_ui(root)
    else:
        print("Unknown item detected. No action taken.")
...
def create_ui():
    ...
    # Add "Detect Item and Remove" button
    model_path = "uecfood_model.tflite"  # Model path
    ...
    detect_button = tk.Button(control_frame, text="Detect Item and Remove",
                              command=lambda: detect_and_remove_item(interpreter, class_labels, root))
